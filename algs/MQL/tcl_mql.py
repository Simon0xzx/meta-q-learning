...
class TCLMQL:
    ...
    def select_action(self, obs, previous_action, previous_reward, previous_obs):
        ...
        latent = self.context(pre_act_rew)
        return self.actor(obs, latent).cpu().data.numpy().flatten()
    ...
    def train_cs(self, task_id = None, snap_buffer = None, train_tasks_buffer = None, adaptation_step = False):
        ...
        with torch.no_grad():   
            ...
            if self.enable_beta_obs_cxt == True:
                snap_ctxt = torch.cat([pos_pxx, self.context(pos_act_rew_obs)], dim = -1).cpu().data.numpy()
                neg_ctxt = torch.cat([neg_xx, self.context(neg_act_rew_obs)], dim = -1).cpu().data.numpy()

            else:
                snap_ctxt = self.context(pos_act_rew_obs).cpu().data.numpy()
                neg_ctxt = self.context(neg_act_rew_obs).cpu().data.numpy()
    ...
    def get_propensity(self, cs_model, curr_pre_act_rew, curr_obs):
        ...
        with torch.no_grad():
            ...
            # batch_size X context_hidden 
            if self.enable_beta_obs_cxt == True:
                ctxt = torch.cat([curr_obs, self.context(curr_pre_act_rew)], dim = -1).cpu().data.numpy()

            else:
                ctxt = self.context(curr_pre_act_rew).cpu().data.numpy()
    ...
    def calc_contrastive_loss(self, replay_buffer):
        ...
        mean_dist = query_norm + key_norm - 2.0 * torch.mm(query_latent, key_latent.T)

        mean_dist = mean_dist - torch.max(mean_dist, axis=1)[0]
    ...
    def do_training(self,
        ...
        critic_prox_out = 0.0
        actor_prox_out = 0.0
        contrastive_loss_out = 0.0
        list_prox_coefs = [self.prox_coef]

        for it in range(iterations):
            contrastive_loss = self.calc_contrastive_loss(replay_buffer)
            ########
            ...
            act_rew_latent = self.context(act_rew)
            pre_act_rew_latent = self.context(pre_act_rew)
            ...
            next_action = (self.actor_target(next_obs, act_rew_latent) + noise).clamp(-self.max_action, self.max_action)
            ...
            target_Q1, target_Q2 = self.critic_target(next_obs, next_action, act_rew_latent)
            target_Q = torch.min(target_Q1, target_Q2)
            target_Q = reward + (mask * self.gamma * target_Q).detach()

            # 2.  Get current Q estimates
            current_Q1, current_Q2 = self.critic(obs, action, pre_act_rew_latent)
        ...
        if iterations == 0:
            ...
            out['contrastive_loss'] = contrastive_loss_out/iterations
    ...
    def adapt(self,
        ...
        out_single = self.do_training(replay_buffer = eval_task_buffer.get_buffer(task_id),
                                      task_buffer=train_tasks_buffer,
